...
height_cloud_uv{it} = z_uv(retrieved_cloud_uv{it});
end
...
for ii=1:length(retrieved_cloud_uv{1})
    fprintf('%8.1f & %3.2f%% &  %3.2f%%  \\\\ \n', ...
        ...
        nanmean(accu_alpha_ss_nc_uv(:,retrieved_cloud_uv{1}(ii))), ...
        nanmean(err_alpha_ss_nc_uv(:,retrieved_cloud_uv{1}(ii))))
end

%% ECSIM SS retrieved with RES correction
fprintf('m from cb & ACU   & ERR  -- ECSIM SS retrieved with RES correction \\ \n')
for ii=1:length(retrieved_cloud_uv{1})
    fprintf('%8.1f & %3.2f%% &  %3.2f%%  \\\\ \n', ...
        z_uv(retrieved_cloud_uv{1}(ii)) - z_uv(retrieved_cloud_uv{1}(1)) , ...
        nanmean(accu_alpha_ss_uv(:,retrieved_cloud_uv{1}(ii))), ...
        nanmean(err_alpha_ss_uv(:,retrieved_cloud_uv{1}(ii))))
end
...
for ii=1:length(retrieved_cloud_uv{1})
    fprintf('%8.1f & %3.2f%% &  %3.2f%%  \\\\ \n', ...
        z_uv(retrieved_cloud_uv{1}(ii)) - z_uv(retrieved_cloud_uv{1}(1)) , ...
        nanmean(accu_alpha_ms_nc_uv(:,retrieved_cloud_uv{1}(ii))), ...
        nanmean(err_alpha_ms_nc_uv(:,retrieved_cloud_uv{1}(ii))))
end

%% ECSIM MS with RES correction
fprintf('m from cb & ACU   & ERR  -- ECSIM MS with RES correction \\ \n')
for ii=1:length(retrieved_cloud_uv{1})
    fprintf('%8.1f & %3.2f%% &  %3.2f%%  \\\\ \n', ...
        z_uv(retrieved_cloud_uv{1}(ii)) - z_uv(retrieved_cloud_uv{1}(1)) , ...
        nanmean(accu_alpha_ms_uv(:,retrieved_cloud_uv{1}(ii))), ...
        nanmean(err_alpha_ms_uv(:,retrieved_cloud_uv{1}(ii))))
end

%% ECSIM MS with SS correction
...
%% ECSIM MS with SS & RES correction
