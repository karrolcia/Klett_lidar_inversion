...
%     id_cb_lidar(it)= id_cb_lidar(it) +1;
    id_cb_radar(it) = find((Ze(it,:)) >= 1.e-3, 1, 'first');
    id_ct_radar(it) = find((Ze(it,:)) >= 1.e-3, 1, 'last');
    I_min(it) = find(beta_atten(it,I_max:end) >=  1.e-4, 1, 'first');
...
%% Calculate beta_atten from Ext
tau_mol_b = zeros(nt,nh);
tau_cld_b = zeros(nt,nh);
for it = 1:nt
    % Calculate molecular optical thickness
    tau_mol_b(it,:)= cumsum(alpha_m(it,:));
    tau_mol_b(it,:)=tau_mol_b(it,:) .* height_res;
    % Calculate cloud optical thickness
    tau_cld_b(it,id_cb_lidar(it):end) =cumsum(Ext(it,id_cb_lidar(it):nh));
%     tau_cld(it,:) =  tau_mol(it,:) +  tau_cld(it,:);
    tau_cld_b(it,id_cb_lidar(it):end) = tau_cld_b(it,id_cb_lidar(it):end) .* height_res;
end
% beta_atten = lid_total ; % Original unit is 1/m/sr, change to 1/km/sr
...
S = zeros (nt,nh);
S(:, id_cb_lidar:end) = 20;
S(:,1:id_cb_lidar-1) = 50;
beta_atten_2 = ((alpha_total./S)+beta_m) .* exp(-2.0 .* (tau_cld_b+tau_mol_b)) ;

test = (beta_atten_2(:,1:I_max+up)-beta_atten(:,1:I_max+up)) ;
% test = beta_atten ./ beta_atten_2
test_mean = mean(mean(test))
test_max = max(max(test))
test_min = min(min(test))
...
plot(alpha(tt,:), height, '.-r')
% herrorbar(alpha(nt,:),sd_alpha(nt,:))
hold on
plot(Ext(tt,:), height, '.-b')
hold on 
plot(alpha_s(tt,:), height, '.-k')
hold on 
plot(alpha_int(tt,:), height_int, '.-m')
hold on
