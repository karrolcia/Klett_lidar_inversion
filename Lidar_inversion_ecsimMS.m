...
% load('fire_27_merged_with_mask_v6_ss.mat')
 ...
 load ('les_1-merged_with_driz_25m.mat' )
 load('true_les_1-merged_with_driz_25m.mat')
...
plot(beta_atten_single(50,1:30),height(1:30),'r',beta_atten(50,1:30),height(1:30),'k')
...
%% Calculating Pp, Pp_u, Pp_l and associated standard deviations in a loop
   for iz = 1:nh
    ...
    % Calculations for Pp - create a function from this       
        ...
        Pp_corr(iz) = S(it,iz) * P_sig_single(it,iz)  * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz));
        Pp(iz) = S(it,iz) * P_sig(it,iz)  * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz));
%         Pp(it,iz) = S * beta_atten(it,iz)  * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz)); 
        sd_Pp(iz) = S(it,iz) * sd_P_sig(it,iz) * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz));
...
%     mu = zeros(it,I_max(it)+down:I_max(it)+up);
  ...
  y_corr(it,:) = log(beta_atten_single(it,I_max(it)+down:I_max(it)+up));
  y(it,:) = log(beta_atten(it,I_max(it)+down:I_max(it)+up));
...
% Estimating value of alpha_o for 3 separate inversions
  alpha_o_p(it) = -0.5 * (slope(it) ./ height_res)...
                  + mean(beta_m(it,I_max(it)+down:I_max(it)+up)) * S(it,I_max(it));
  alpha_o_p_corr(it) = -0.5 * (slope_corr(it) ./ height_res)...
                  + mean(beta_m(it,I_max(it)+down:I_max(it)+up)) * S(it,I_max(it));
...
figure
plot(alpha(1,1:25),height(1:25),'.-k')
hold on
plot(Ext(1,1:25), height(1:25), '.-b')
hold on 
plot(alpha_corr(1,1:25), height(1:25), '*-r')
