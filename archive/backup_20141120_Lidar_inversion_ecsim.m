...
% Find the peak of the attenuated backscatter
 ...
 [Peak_depol, D_max] = max(lin_depol(:,1:perp_max), [], 2) ;
 
 for icb = 1:nt
  id_cb_lidar(icb) = find (perp(icb,1:perp_max(icb)) >= (Peak_perp(icb)/10), 1,'first');
%   id_ct_lidar(icb) = find (perp(icb,perp_max(icb):I_max(icb)) ~= 0, 1,'last');
  id_cb_radar(icb) = find((Ze(icb,:)) >= 1.e-4, 1, 'first');
 end
 
 id_cb_lidar =  id_cb_lidar';
...
up = 5;
 ...
 % Integrating attenuated backscatter
 
 %% Corecting signal for the multiple scattering
 
perp_int = zeros(nt,nh);
para_int = zeros(nt,nh);
beta_atten_int = zeros(nt,nh);
% % single_scattering = zeros(nt,nz);
lower_bound = id_cb_lidar;
upper_bound = I_max+up;
int_limit = (upper_bound-lower_bound);
perp_sum = zeros(nt,nh);

para_sum = zeros(nt,nh);
beta_atten_sum = zeros(nt,nh);
beta_atten_int_all = zeros(nt,nh);
beta_atten_sum_all = cumsum(beta_atten,2);

for it= 1:nt
    perp_sum (it,lower_bound(it):upper_bound(it)) = cumsum( perp(it, ...
        lower_bound(it):upper_bound(it)));
    para_sum (it,lower_bound(it):upper_bound(it)) = cumsum( para(it, ...
        lower_bound(it):upper_bound(it)));
    beta_atten_sum (it,lower_bound(it):upper_bound(it)) = cumsum( beta_atten(it, ...
        lower_bound(it):upper_bound(it)));
    for jz = 0:length(lower_bound(it):upper_bound(it))
        perp_int(it,lower_bound(it)+jz) = perp_sum(it,lower_bound(it)+jz) .* ...
             length(perp(lower_bound(it):lower_bound(it)+jz)) ;
        para_int(it,lower_bound(it)+jz) = para_sum(it,lower_bound(it)+jz) .* ...
             length(para(lower_bound(it):lower_bound(it)+jz));
        beta_atten_int(it,lower_bound(it)+jz) = beta_atten_sum(it,lower_bound(it)+jz) .* ...
             length(beta_atten_sum(lower_bound(it):lower_bound(it)+jz));
    end
    
    for jz = 1: length(alt)
        beta_atten_int_all(it,jz) = beta_atten_sum_all(it,jz) .*  ...
            length(beta_atten_sum_all(it,1:jz)) ;
    end
end

% correction_factor = ((1 - perp./para).^2) ./ ((1 + perp./para).^2) ;
% single_scattering = correction_factor .* beta_atten ;
correction_factor = ((1 - perp_int./para_int).^2) ./ ((1 + perp_int./para_int).^2) ;
single_scattering = correction_factor .* beta_atten ;
corrected_signal = beta_atten ;
corrected_signal(:,lower_bound:upper_bound) = single_scattering(:,lower_bound:upper_bound);


plot(single_scattering(50,:),height,'r',beta_atten_int_all(50,:),height,'k')
...
%% Prelocating matrices
  ...
  Pp=zeros(nt,nh)                 ; %P' See Eq.(11)
  Pp_u=zeros(nt,nh)               ; %P' corresponding to S+dS 
  Pp_l=zeros(nt,nh)               ; %P' corresponding to S-dS

  sd_Pp=zeros(nt,nh)              ; %Estimated sd of P'
  sd_Pp_u=zeros(nt,nh)            ; %..
  sd_Pp_l=zeros(nt,nh)            ; %..
...
%% Calculating Pp, Pp_u, Pp_l and associated standard deviations in a loop
   for iz = 1:nh
    ...
    % Calculations for Pp - create a function from this       
        ...
        Pp_corr(it,iz) = S(it,iz) * corrected_signal(it,iz)  * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz));
        Pp(it,iz) = S(it,iz) * beta_atten(it,iz)  * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz));
%         Pp(it,iz) = S * beta_atten(it,iz)  * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz)); 
        sd_Pp(it,iz) = S(it,iz) * sd_beta_atten(it,iz) * exp(-2.0 * int_S_beta) * exp(2 * tau_mol(it,iz));
...
% Estimating value of alpha_o for 3 separate inversions
  alpha_o_p(it) = -0.5 * (slope(it) * 1/mean(Pp(it,I_max(it)+down:I_max(it)+up))) ...
                  + mean(beta_m(it,I_max(it)+down:I_max(it)+up)) * S(it,iz);
  alpha_o_p_corr(it) = -0.5 * (slope_corr(it) * 1/mean(Pp_corr(it,I_max(it)+down:I_max(it)+up))) ...
                  + mean(beta_m(it,I_max(it)+down:I_max(it)+up)) * S(it,iz);
...
%     sd_Pp(it,I_max(it)+down:I_max(it)+up), height_norm_1(it), height_norm_2(it), alpha_o_p(it)) ;
  [alphap(it,:), sd_alphap] = Klett_inversion(height, Pp(it,:), sd_Pp(it,:), ...
      height_norm_1(it), height_norm_2(it), alpha_o_p(it)) ;
  [alphap_corr(it,:), sd_alphap_corr] = Klett_inversion(height, Pp_corr(it,:), sd_Pp(it,:), ...
      height_norm_1(it), height_norm_2(it), alpha_o_p_corr(it)) ;
 ...
 %% Transforming alpha_p and sd_alpha_p to alpha and sd_alpha
  ...
  sd_alpha(it,:) = sd_alphap ;%- S * beta_m(it,:) ;
