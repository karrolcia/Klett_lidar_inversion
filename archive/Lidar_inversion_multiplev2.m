...
%     norm_down(it) = find(beta_atten(it,:) >=  1.e-6, 1, 'last');
...
for it= 1:nt
    ...
    for jz = 1:length(id_cb_lidar(it)-2:id_ct_radar(it))
...
for i = 1:nt
    ...
    bias(i) = alpha_slope (i) -(Ext(i,io(i)) + S(it, io(it)) .* beta_m(it, io(it)));
end
mean_bias = mean(bias)
...
for ii=1:6
    fprintf('%8.0f & %8.2f &  %8.6f &  %8.2f & %8.6f &  %8.6f &  %8.6f  \\\\ \n', ...
